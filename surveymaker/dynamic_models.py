...
def get_survey_response_model(survey, regenerate=False, notify_changes=True):
    ...
    model = type('Response'+str(name), (models.Model,), attrs)
    print("model: ", model)
...
def generate_model_hash(survey):
    ...
    """
    return sha256(survey.get_hash_string().encode('utf-8')).hexdigest()
