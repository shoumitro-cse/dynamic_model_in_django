...
def get_cached_model(app_label, model_name, regenerate=False, get_local_hash=lambda i: i._hash):
    ...
    # previous_model = apps.get_model(app_label, model_name)
    previous_model = None
    ...
    if previous_model is not None and not regenerate:
        ...
        if cache.get(CACHE_KEY) != get_local_hash(previous_model):
            logging.debug("Local and shared dynamic model hashes are different: %s (local) %s (shared)" %
                          (get_local_hash(previous_model), cache.get(CACHE_KEY)))
            regenerate = True
...
# for south db
# https://south.readthedocs.io/en/latest/databaseapi.html
